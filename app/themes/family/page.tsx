...
const guideItems = [
  {
    ...
    hotel: "리가 로얄 호텔 오사카",
    hotelId: "rihga-royal-hotel-osaka",
  },
  {
    condition: "전망과 직결 동선까지 챙긴 효도 여행이라면",
    hotel: "오사카 메리어트 미야코 호텔",
    hotelId: "osaka-marriott-miyako-hotel",
  },
  {
    ...
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <header className="sticky top-0 z-20 bg-white border-b border-gray-100">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <span className="text-sm font-semibold text-gray-900">가족·부모님 동반용</span>
        </div>
      </header>
      ...
      <main className="max-w-lg mx-auto px-4">
        ...
        <section className="pt-7 pb-5">
          <h1 className="text-[1.45rem] font-bold text-gray-900 leading-tight mb-2.5">
            가족 또는 부모님과 함께라면<br />먼저 보기 좋은 오사카 숙소
          </h1>
          <p className="text-[0.9rem] text-gray-500 leading-relaxed">
            아이 동반, 부모님 동반, 넓은 객실, 이동 편의,<br />
            안정적인 숙박 경험까지 고려해 함께 가는 사람이<br />
            만족하기 쉬운 숙소부터 골랐어요.
          </p>
        </section>
        ...
        <section className="mb-4">
          <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4">
            <p className="text-xs font-semibold text-amber-600 mb-3 tracking-wide">빠른 선택 가이드</p>
              ...
              {guideItems.map((item) => (
                <li key={item.hotelId} className="flex items-start gap-2 text-sm">
                  ...
                  <span className="text-gray-600">
                    {item.condition}{" "}
                    <span className="font-semibold text-amber-700">{item.hotel}</span>
                  </span>
                </li>
              ))}
            </ul>
        ...
        <section className="mb-4">
          <p className="text-xs text-gray-400 leading-relaxed px-1">
            객실 안정감, 이동 편의, 부모님·아이 동반 적합성, 체크인 스트레스 감소를 기준으로
            먼저 보기 좋은 순서로 정리했어요.
          </p>
        </section>
            ...
            {familyHotels.map((hotel) => (
              ...
              >
                ...
                <div className="px-5 pt-4 pb-3">
                  ...
                  {/* 선택 이유 — 핵심 메시지 */}
                  <p className="text-[0.9rem] font-semibold text-amber-700 leading-snug mb-3">
